...
# For testing: If nginx don't start build
if [ -e /usr/bin/nginx ]; then
  echo "NGINX already present - No compile required"
  exit 0
fi
...
./configure --with-mail --with-mail_ssl_module --with-stream --with-http_ssl_module --with-ipv6 --prefix=/tmp/nginx --sbin-path=/nginx --conf-path=/tmp/nginx/nginx.conf --error-log-path=stderr --pid-path=/tmp/nginx/nginx.pid

header "Building NGINX. This takes some time  ..."
make
